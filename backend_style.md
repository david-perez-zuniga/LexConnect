...
Usa `app.core.logger` en lugar de `print()`. El logger ya está configurado con formato y salida estándar:
...
logger.error(f"Error con el server: {e}")
...
12. **Usa `app.core.logger` para logging** — importa `logger` y usa `logger.info()`, `logger.error()`, `logger.exception()`
